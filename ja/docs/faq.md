...
ストレージシステムの増強にともないグループ領域の構成が変更され、2020年度まで使用していたグループ領域(以下、**旧領域**)から新しいグループ領域(以下、**新領域**)へのデータ移行を行います。

2021年5月に新規導入された計算資源([計算ノード(A)](#q-what-is-the-difference-between-compute-node-a-and-compute-node-v))からは**旧領域**にアクセスすることができません。このため、計算ノード(A)を含むすべての計算ノードおよびインタラクティブノードからアクセス可能な**新領域**に**旧領域**にあったデータをコピーし、その後、**旧領域**のあったパスを移行先へのシンボリックリンクに置き換える作業を行っています。
