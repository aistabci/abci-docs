...
* ABCI 2.0インタラクティブノードから、グループ領域(`/groups`)、高速データ領域(`/bb`)、ABCIクラウドストレージ等のABCI 2.0 ストレージは引き続きアクセスできます。ただし、ストレージは段階的に終了します。終了時期は「[今後のストレージサービスのお知らせ](https://abci.ai/news/2024/09/04/ja_storage_after_nov2024.html)」を参照してください。
...
* ABCI 2.0インタラクティブノードにABCI 3.0のグループ領域を`/groups-new`のマウントポイントでマウントします。
    * `/groups`、`/bb`等のABCI 2.0ストレージ上の必要なデータは、インタラクティブノードから利用者自身でABCI 3.0のグループ領域(`/groups-new`)へ移行してください。
...
以下の申請は受け付けます。
ただし、ABCI 2.0システムには反映しますが、ABCI3.0へは反映しません。

* クラウドストレージ管理

以下の申請は受け付けます。
ただし、ABCI 2.0システムには反映しますが、ABCI3.0へ反映するためには別途申請が必要です。ABCI 3.0へ反映するための申請手順は、後日お知らせします。
...
以下は申請を受け付けません。
利用者ポータルでは、下記サービスの申請ボタンは非表示となります。申請しても承認されません。
...
利用者が利用可能な機能は、下記のとおりです。

* すべての利用者：
    * パスワード再発行
    * 公開鍵の参照、削除（★ABCI3.0への反映方法は別途ご案内いたします。）
* 管理者権限がある利用者：
    * 利用者削除（★）
    * 利用者編集（★）
        * 利用管理者権限設定（★）
        * 利用責任者変更（★）
        * 請求書送付先・予算コード変更（★）
    * ストレージ関連の申請（クラウドストレージの申請を含む）（★）
    * ポイント使用履歴の参照（2025/3/30 17:00までABCI2.0のポイント使用履歴（2024/10/31までの履歴)を参照可能（ABCI 3.0でのポイント使用履歴は参照不可））
