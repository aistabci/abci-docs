...
Dockerfileから作成したSingularityイメージファイルをABCIシステム上で利用する2つの方法を示します。ABCIシステムでは、システム内で公開されているDockerイメージのみDockerコマンドで利用可能なため、別システム等で構築した環境をABCIシステム上で利用するためにはSingularityコンテナを利用する必要があります。

#### Docker Hubを経由した作成方法 {#build-via-dockerhub}

Dockerの実行環境があるシステム上でDockerコンテナイメージを作成し、Docker Hubにアップロードすることで、ABCIシステム上で作成したDockerコンテナイメージを利用します。
以下の例では、Pytorchで[SSD300 v1.1モデル](https://github.com/NVIDIA/DeepLearningExamples/tree/master/PyTorch/Detection/SSD)を学習するDockerfileからDockerコンテナイメージを作成し、アップロードしています。
...
#### DockerfileをSingularityレシピに変換する作成方法 {#build-by-singularity-recipe}
[singularity-cli](https://github.com/singularityhub/singularity-cli)はSingularityレシピとDockerfileの変換を実施するPythonAPIです。

!!!warning
    ただしDockerfileの記述方法によっては変換したレシピファイルでは`Singularity build`でエラーが発生する可能性があります。

singularity-cliのインストール例）
...
以下の例ではPytorchで[SSD300 v1.1モデル](https://github.com/NVIDIA/DeepLearningExamples/tree/master/PyTorch/Detection/SSD)を学習するDockerfileを編集し、singularity-cliによるrecipeファイルの変換を行います。オリジナルのDockerfileではpipのPATHが通っていないこと、WORKDIRにファイルがコピーされないことの2点でエラーが発生するため、PATHの追加並びにCOPY先を相対パスではなくWORKDIRの絶対パスへの変更を行っています。
...
(work) [username@es1 SSD]$ diff Dockerfile
3a4,6
> # Add pip Path
