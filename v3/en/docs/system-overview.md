...
Among them, each interactive node and compute node are equipped with InfiniBand HDR (200 Gbps) and are connected to Storage Systems described later by InfiniBand switch group.
Also, each compute node is equipped with 8 port of InfiniBand NDR (200 Gbps) and the compute nodes are connected by InfiniBand switch.
