...
ジョブを投入/実行する際には、利用したい資源タイプ名と数量を指定します。ABCIシステムでは[計算ノード](01.md#compute-node)と[メモリインテンシブノード](01.md#memory-intensive-node)の2種類の計算リソースが存在し、その各々について以下表のように資源タイプが用意されています。
