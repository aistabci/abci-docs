...
| -l USE_BEEOND= | Only when using resource type Full, you can submit a job that use the BeeGFS On Demand (BeeOND). Please refer to [4.3.Local Storage](04.md#local-storage) for details.
| -v GPU_COMPUTE_MODE= | Only when using resource type Full, you can change GPU compute mode. Please refer to [GPU COMPUTE MODE change](03.md#gpu-compute-mode-change) for details.
