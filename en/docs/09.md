...
### Using user Docker image {#use-another-docker-image}

There are two ways to use user Docker image.

**Using public Docker Hub**

Make Docker container image, and upload Docker Hub.

```
[user@pc ~]$ mkdir docker_dir
[user@pc ~]$ cd docker_dir
[user@pc docker_dir]$ cat Dockerfile
FROM centos
RUN yum install -y iptables-services
[user@pc docker_dir]$ docker build -t user/docker_name
[user@pc docker_dir]$ docker login && docker push user/docker_name
```

Singularity container started by `singularity run` command.

```
[username@es1 ~]$ qrsh -l rt_F=1 -l h_rt=1:00:00
[username@g0001~]$ module load singularitypro/3.5
[username@g0001~]$ singularity run --nv docker://user/docker_name:latest
...
Singularity> iptables --version
iptables v1.8.4 (nf_tables)
```

**conversion Dockerfileto Singularity recipe**

Installation of singularity-cli

```
[username@es1 ~]$ git clone https://www.github.com/singularityhub/singularity-cli.git
[username@es1 ~]$ cd singularity-cli
[username@es1 singularity-cli]$ python3 setup.py install
```

conversion to recipe file by singularity-cli

```
[username@es1 ~]$  singularity-cli/spython recipe Dockerfile singularity.def
```

Create a Singularity image file using `build`

```
[username@es1 ~]$ module load singularitypro/3.5
[username@es1 ~]$ singularity build --fakeroot singularity.sif singularity.def
INFO:    Starting build...
(snip)
INFO:    Creating SIF file...
