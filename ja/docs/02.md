...
### メンバーの種類 {##member-type}
ABCIグループに所属するメンバーには、「利用責任者」「利用管理者」「利用者」の3種類があります。ABCIシステムを利用するには、「利用責任者」が [ABCI利用ポータル](https://portal.abci.ai/user/project_register_app.php) からABCIグループ申請・利用者(「利用管理者」もしくは「利用者」)を行い、ABCIアカウントを取得する必要があります。
...
### ABCIアカウントの一意性
1人の方は原則として1つのABCIアカウントを取得していただくことになります。会社などで1つのABCIアカウントを取得して複数の社員が共有して使い回すことは認められません。
一方で1人の方が複数の団体に所属しており、それぞれ異なった肩書をお持ちの場合は複数のABCIアカウントを取得することができます。この場合、それぞれの肩書に対応するABCIアカウントを使い分けていただく必要があります。

また、1人の方が1つの団体の中で複数のテーマに同時に取り組んでいる場合、1人で複数のABCIアカウントをそれぞれ取得するのではなく、1つのABCIアカウントで複数のABCIグループに所属するメンバーとなります。この場合、その利用目的に応じてABCIグループを使い分けていただく必要があります。
