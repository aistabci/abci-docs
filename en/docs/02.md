...
To use the ABCI system, the "Responsible Person" must apply for a new "ABCI application" on the [ABCI User Portal](https://portal.abci.ai/user/project_register_app.php?lang=en) and obtain an ABCI group and one or more ABCI accounts. See the [ABCI Portal Guide](https://docs.abci.ai/portal/en/) for more details.
