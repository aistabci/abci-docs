...
On-demand and Spot services allow you to specify a priority when running a job, as follows:
...
In Spot service, you can specify `-400` to your job, so as to execute it in higher priority to other jobs. However, [Billing](#accounting) is performed according to the POSIX priority coefficient.
