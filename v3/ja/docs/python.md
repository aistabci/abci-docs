...
!!! warning
    無効化実行後は、該当クラウドストレージアカウントディレクトリに存在する全てのバケットに以下2点の設定が行われます。

    - 全IPアドレスからのアクセスを拒否するバケットポリシーの適用
    - バケットACLの初期化
...
無効化が終了したかどうかについては [お問い合わせ](https://docs.abci.ai/v3/ja/contact/) をご参照の上、abci3-qa@abci.ai へメールで質問してください。
