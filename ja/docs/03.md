...
|:--|:--|:--|:--|
| On-demand | インタラクティブジョブの実行サービス。プログラムのコンパイルやデバッグ、対話的なアプリケーション、可視化ソフトウェアの実行に適しています。 | 1.0 | [インタラクティブ利用](#interactive-jobs) |
| Spot | バッチジョブの実行サービス。対話的な処理の必要がないアプリケーションの実行に適しています。On-demandサービスに比べてより長時間のジョブや、より並列度の高いジョブの実行が可能です。 | 1.0 | [バッチ利用](#batch-jobs) |
| Reserved | 事前予約型ジョブサービス。計算ノードを日単位で事前に予約して利用できるサービスです。On-demandおよびSpotサービスの混雑の影響を受けることなく、計画的なジョブ実行が可能となります。また、経過時間およびノード時間積の制限がないため、より長時間のジョブ実行が可能です。 | 1.5 | [事前予約](#reservation)、[インタラクティブ利用](#interactive-jobs)、[バッチ利用](#batch-jobs) |
