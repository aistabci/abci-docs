...
計算リソースを日単位で事前に予約して利用できるサービスです。On-demandおよびSpotサービスの混雑の影響を受けることなく、計画的なジョブ実行が可能となります。また、Reservedサービスの予約日数が長い場合には、経過時間制限をSpotサービスより長く設定可能であるため、Spotサービスより経過時間の長いジョブ実行が可能です。
...
!!! note
    Reservedサービスにおいては、経過時間制限指定をしないことで無制限でジョブを投入可能ですが、予約の終了と共にジョブは強制終了します。Reservedサービスに関する制限の詳細については[事前予約](#advance-reservation)を参照してください。
...
| システムあたりの最大同時予約可能ノード数 | 476ノード | 50ノード |
