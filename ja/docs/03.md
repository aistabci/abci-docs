...
| -l USE_BEEOND= | 資源タイプFullを利用した場合、BeeOND を利用するジョブを投入することが可能です。詳細は[4.3.ローカルストレージの利用](04.md#local-storage) を参照ください。 |
| -v GPU_COMPUTE_MODE=  | 資源タイプFullを利用した場合、GPUのCOMPUTE MODEを変更が可能です。詳細は[GPUのCOMPUTE MODE変更](03.md#gpucompute-mode)を参照ください。
...
| モード | 説明 |
|:--|:--|
|-v GPU_COMPUTE_MODE=0 | 0はDEFAULTモードです。<BR>1つのGPUを複数のプロセスから同時に利用できます。 |
|-v GPU_COMPUTE_MODE=1 | 1はEXCLUSIVE_PROCESSモードです。<BR>1つのGPUを1プロセスのみが利用できます。1プロセスから複数スレッドの利用は可能です。 |
