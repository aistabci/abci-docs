...
Singularity Pro 3.5以降では、`fakeroot`を使ったイメージ構築が可能です。

!!! warning
    GPFS配下でやると権限の問題でエラーが発生します。以下のように、`newgrp`コマンドを実施いただくことで回避可能です。

実行例）
```
[username@es1 singularity]$ newgrp $group_id
[username@es1 singularity]$ module load singularitypro/3.5
[username@es1 singularity]$ singularity build --fakeroot test.sif test.def
(snip)
