...
* これまで資源タイプごとに割り当てられるノードに制限はなく、利用可能な計算リソースから順次割り当てていました。<br>
  2024年4月12日以降、資源タイプごとに割り当てられるノードに制限を設けます。<br>
...
* Reservedサービスにおいて、2024年4月12日以降、予約を割り当てるノードに制限を設けます。<br>
下記に記載のノードから予約が割り当てられるようになります。<br>
ただし、予約した計算ノードで実行できる資源タイプに制限はありません。<br>
以前と同様に、計算ノード(V)は資源タイプrt_F, rt_G.large, rt_G.small, rt_C.large, rt_C.smallを実行でき、計算ノード(A)は資源タイプrt_AF, rt_AG.smallを実行できます。

| ノード種類 | ノード数 | ホスト名 |
| :-- | :-- | :-- |
| 計算ノード(V) | 1,054 | g0001-g1020,g1055-g1088 |
| 計算ノード(A) | 115 | a0001-a0115 |
