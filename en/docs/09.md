...
How to run built Docker container image on ABCI, please refer [Running a container with Singularity](#running-a-container-with-singularity)

#### Conversion Dockerfile to Singularity recipe {#build-by-singularity-recipe}

To convert Dockerfile to Singularity recipe makes it possible to build Singularity container image on ABCI.
You can convert Dockerfile to Singularity recipe by [Singularity Python](https://singularityhub.github.io/singularity-cli/).
...
WORKDIR /workspace
ENV PYTHONPATH "${PYTHONPATH}:/workspace"
COPY requirements.txt /workspace         <- Copy destination from relative path (.) to absolute path
RUN pip install --no-cache-dir git+https://github.com/NVIDIA/dllogger.git#egg=dllogger
RUN pip install -r requirements.txt
RUN python3 -m pip install pycocotools==2.0.0
# Copy SSD code
COPY ./setup.py /workspace               <- Copy destination from relative path (.) to absolute path
COPY ./csrc /workspace/csrc              <- Copy destination from relative path (.) to absolute path
RUN pip install .
COPY . /workspace                        <- Copy destination from relative path (.) to absolute path
