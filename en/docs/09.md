...
### Create a Singularity image (pull) {#create-a-singularity-image}
...
### Create a Singularity image (build) {#build-a-singularity-image}

In the SingularityPRO environment of the ABCI system, you can build container image files using `fakeroot` option.

!!! note
    In the SingularityPRO environment, you can also build container image file using remote build. See [this page](abci-singularity-endpoint.md) for more information.
...
### Running a container with Singularity {#running-a-container-with-singularity}
...
#### Convert Dockerfile to Singularity recipe {#build-by-singularity-recipe}
