...
To use an ABCI account, determine the research and development theme according to the agreements of use (rules or regulations) posted in the "Application Procedure", and submit the "ABCI Usage Application" from the ABCI Usage Portal. The ABCI application will be reviewed based on the terms of use, and if requirements are met, an ABCI group for which the application was made will be created and the applicant will be notified that the application has been adopted. After receiving notification, the applicant may start using the account.
...
To use the ABCI system, the "Responsible Person" must apply for a "user group" from the [ABCI User Portal](https://portal.abci.ai/user/project_register_app.php?lang=en) and obtain an ABCI account. See the [ABCI Portal Guide](https://docs.abci.ai/portal/en/) for more details.
