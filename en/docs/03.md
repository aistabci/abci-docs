...
|Reserved|Job servive of reservation|1.5|Batch/Interactive|
...
When a job finishes, the ABCI point is caluculated again by actual elapsed time, and repayment process is executed.
...
The repayment process is not executed unless reservation is calcelled.
...
The available resource type and number for each services are follows.
...
|         |rt_C.large|1|
|         |rt_C.small|1|
|Reserved |rt_F      |1-number of reserved node|
|         |rt_G.large|1|
|         |rt_G.small|1|
|         |rt_C.large|1|
|         |rt_C.small|1|

The job limit of elapsed time for each services are as follows.
...
In case of Spot or On-demand servie, the job cannot be executed over the node-hour restriction bellow.
...
```
Using ABCI point = Service charge coeffcient x Resource type charge coefficient 
...
!!! note
    ...
    * If elapsed time of job executing is less than minimum elapsed time, ABCI point caluculated based on minimum elapsed time.

The calculation formula of ABCI point using for Reserved service is follows.

```
Using ABCI point = Service charge coefficient x number of reserved node 
...
|-N *name*|Specify job name. default is name of jobscript.|
...
|-hold_jid *job_id*|Specify job ID having dependency. the submitted job is not executed until dependend job finished.|
|-ar *ar_id*|Specify reserved ID(AR-ID),when using reserved comput node|
...
The job script is descrived job execute option, such as resource type, elapsed time limit,etc., and executing command sequence.
...
Standard output generated during a job execution is written to a standard output file and error messeges generated during the
...
The ABCI poing consumed when complete reservation.
