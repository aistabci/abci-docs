...
To avoid the problems, this example modifies the Singularity recipe after conversion.

- Files in WORKDIR will not be copied => Set the copy destination to the absolute path of WORKDIR
- No path to pip => Add the setting to take over environment variables in the %post section
