...
* Using as a local scratch of a node (*Using as a local scratch*, *Using as a reserved persistent local storage*).
...
| using as a Reserved persistent local storage  | ok | - |
...
### Using as a persistent local storage
...
!!! warning
    The files created under $SGE _ ARDIR` will be deleted when the Reserved service is finished, so the necessary files should be copied to the home area or group area using `cp` command etc.

!!! warning
    The compute node (A) contains two NVMe SSDs with separate local storage for local scratch and reserved service storage.
    Since the compute node (V) contains only one NVMe SSD, the local storage for local scratch usage and the local storage for reserved service storage usage are the same storage.
    Pay attention to usage when using Reserved Service Storage.
