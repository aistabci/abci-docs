...
各ジョブサービスでは、実行時に優先度を指定できます。指定可能な優先度の値は以下のとおりです。

| サービス名 | 優先度 | 説明 | サービス課金係数 |
|:--|:--|:--|:--|
| On-demand | 1 | 既定 (変更不可) | 1.0 |
| Spot      | 0 | 既定            | 1.0 |
|           | 1 | 優先実行        | 1.5 |
| Reserved  | 0 | 既定 (変更不可) | 1.5 |

On-demandサービスでは、優先度は`1`に固定されており変更できません。

Spotサービスでは、`1`を指定することで他のジョブより優先的に実行できます。ただし、サービス課金係数に応じた課金が行われます。

Reservedサービスでは、インタラクティブジョブ、バッチジョブのいずれの場合も優先度は`0`に固定されており変更できません。
