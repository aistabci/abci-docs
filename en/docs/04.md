...
Home area is the disk area of the Lustre file system shared by each interactive and compute node, which is available to all ABCI users by default. The disk quota is limited to 200GiB.
...
Group area is the disk area of the GPFS file system shared by each interactive and compute node.
...
In ABCI System, 1.6 TB NVMe storage is installed into each compute node and 1.8 TB SATA 3.0 storage is installed into each memory intensive node. There are two ways to utilize this storage as follows:
...
The local storage installed into each compute node and memory intensive node is available as Local area without specifying any additional options.
...
The local storage path is different for each job and user can access to local storage by using [environment variables](03.md#environment-variables) `SGE_LOCALDIR`.
...
distributed shared file system BeeGFS On Demand(BeeOND) on demand.
