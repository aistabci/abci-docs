...
## 4.2.1 グループ領域のファイルの所有者変更 {#chown-group-area-files}

グループの利用管理者は、グループから削除された利用者が所有していたグループ領域のファイルの所有者を自分自身に変更することが可能です。
所有者の変更は、`abci_chown`コマンドを使用します。

```
$ abci_chown --from <username> [-R] filepath|dirpath [filepath|dirpath]...
```

| オプション | 説明 |
|:--|:--|
| --from *username* | グループに所属していないユーザを指定する |
| -R | 再帰的に実行する |
...
計算ノードとメモリインテンシブノードのNVMeストレージは、そのまま計算ノードの
ローカルストレージとして利用できます。
...
BeeONDストレージは、ジョブに割り当てられた計算ノードのローカルストレージで構成した分散共有ファイルシステム(BeeGFS)のストレージ領域です。
