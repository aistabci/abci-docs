...
今まで、 `/groups[1-2]/gAA50NNN`（旧領域）をご利用の皆様には、4月から新しく `/groups/gAA50NNN`（新領域）が利用可能になります。
それぞれ、すべてのインタラクティブノードとすべての計算ノードで、利用可能なディレクトリです。

旧領域を、今後、新領域へデータ移行します。
/groups2, /groups1, /fs3 の順にデータ移行予定、完了まで約1年の予定です。

以下にデータ移行について説明します。

**(1) 新領域/groups/gAA50NNNについて**

* 新領域には、旧領域に設定していたディスククォータの2倍まで書き込み可能です。データ移行完了までの一時的な増量です。
* 旧領域のファイルの実体は、新領域の `/groups/gAA50NNN/migrated_from_SFA_GPFS/` にコピーされる予定です。
* データ移行進捗に伴い旧領域のファイルが新領域にコピーされますが、データ移行完了までの期間は、利用者様から参照不可・アクセス不可です。参照不可ですがクォータの利用量は増えますので、ご注意ください。
* データ移行完了後は、元のクォータ値に戻します（猶予期間を設ける予定です）。

**(2) 旧領域/groups[1-2]/gAA50NNNについて**

**1. データ移行中**

* 運用側がデータ移行するため、利用者から見るとシステム側が自動的にデータ移行を実施しているように見えます。
* データ移行中に、利用者様は自由に旧領域にR/W可能、ファイルの削除も可能です。
* データ移行する量を減らすために、旧領域には、可能な限り新規ファイルを置かないでください。不要なファイルは削除してください。可能な範囲で構いません、ご理解お願いいたします。（なお、データ移行完了後は、この制限は無くなります。）

**2.データ移行完了のための参照不可期間について**

* データ移行完了の確認のために、旧領域を参照不可の期間が数日発生する予定です。
* その期間、新領域は読み込み/書き込み/削除可能予定です。
* 今までと同じパス `/groups[1-2]/gAA50NNN` でアクセス可能にするために、運用側がシンボリックリンクを変更します。

**3.データ移行完了後について**

* 今までと同じパス `/groups[1-2]/gAA50NNN` でアクセス可能です。
* 旧領域のファイルの実体は、新領域の `/groups/gAA50NNN/migrated_from_SFA_GPFS/` にコピーされた状態になる予定です。
