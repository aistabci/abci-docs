...
To cancel a reservation, use the `qrdel` command or the ABCI User Portal.`qrdel` command can specify multiple ar_id as a comma-separated list.If at least one reserved ID that does not exist or that does not have delete authority is specified in the specified ar _ id, an error occurs and the delete is not performed.
