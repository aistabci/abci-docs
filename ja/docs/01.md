...
## 1.2. 計算リソースのハードウェア概要 {#computing-resources}
...
ABCIシステムは、人工知能やビッグデータ処理に用いるデータを格納するための共有ファイルシステム、及びABCIクラウドストレージを備えており、合算で約40 PBの容量があります。インタラクティブノード、計算ノード、メモリインテンシブノードは、共有ファイルシステムをマウントしており、利用者は共通のマウントポイントからこれらのファイルシステムにアクセスすることができます。
...
| グループ領域3 | /groups3 | 6.6 PB | GPFS | 特定用途に予約済みの領域 |
| アプリケーション領域 | /apps | 335 TB | GPFS | 共通ソフトウェア・データ等で利用する領域 |
...
| 計算ノード | /local | 1.5 TB | XFS | [ローカルストレージ](04.md#local-storage)を参照 |
| メモリインテンシブノード | /local | 1.8 TB | XFS | [ローカルストレージ](04.md#local-storage)を参照 |
