...
ABCIシステムのフロントエンドであるインタラクティブノード(代表ホスト名：*es*)にログインするには、[ABCI利用ポータル](https://portal.abci.ai/user/)にてSSH公開鍵を登録し、SSH公開鍵認証を用いてアクセスサーバ(ホスト名：*as.abci.ai*)にログインして、SSHポートフォワーディングによるトンネリング（以下「SSHトンネル」という）を設定する必要があります。

なお本章では、ABCIのサーバ名は *イタリック* で表記します。

### 2.2.1. SSHクライアントによるログイン {#ssh-client-login}

ほとんどのLinux、macOSを含むUNIX系OS、Windows 10 April 2018 Update (1803)以降のWindows 10には、デフォルトでSSHクライアントがインストールされています。

以下では、アクセスサーバでSSHトンネルの設定後にインタラクティブノードにログインする方法と、より簡便なProxyJumpを用いたログイン方法を説明します。

#### 標準的なログイン方法 {#standard-method}

以下のコマンドでアクセスサーバ(*as.abci.ai*)にログインし、SSHトンネルを設定します。
...
!!! warning
    Windows 10 バージョン1803以降で標準でバンドルされているOpenSSH_for_Windows_7.7p1などでは、ProxyJumpおよびProxyCommandが使えません。Windows Subsystem for Linux (WSL)にバンドルされているOpenSSHでは使えます。
