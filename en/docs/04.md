...
!!! warning
    Home area has the upper limitation of files in one directory.
    The maximum number of files depends on the length of filename located in the directory
    and it is approximately from 4 M to 12 M.
    The error “No space left on device” will happen and fail to create a file, when exceeding the limit.
