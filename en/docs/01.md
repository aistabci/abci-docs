...
In the ABCI system, there are two types of compute nodes (compute node(V) and compute node(A)). In order to improve the convenience of program development for each compute node, two types of interactive nodes (interactive node(V) and interactive node(A)) are also provided.
The program development of the application for the compute node(V) is carried out in the interactive node(V), and the program development of the application for the compute node(A) is carried out in the interactive node(A).
It is possible to submit a job from the interactive node(V) to the compute node(A). Similarly, It is possible to submit a job from interactive node(A) to the compute node(V).
...
| Group area | /groups | 10.8PB | Lustre | See [Group Area](04.md#group-area) |
