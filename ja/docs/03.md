...
資源タイプFullを利用した場合、GPUのCOMPUTE MODEを変更することができます。利用可能なモードは以下の3つです。
...
インタラクティブジョブで利用する場合は、ジョブを投入する際に `-v GPU_COMPUTE_MODE=[利用するモード]` を指定します。

```
[username@es1 ~]$ qrsh -g grpname -l rt_F=1 -l h_rt=1:00:00 -v GPU_COMPUTE_MODE=[利用するモード]
```
