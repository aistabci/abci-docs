...
!!! Warning
    ホーム領域やグループ領域等、ローカルストレージ以外の領域は利用者全員で共有している資源です。共有の資源については、下記に留意して利用してください。

    * 他の利用者の迷惑となるため、過度のI/O負荷をかけないでください。I/Oベンチマーク等の実施は避けてください。
    * inodeの大量消費は避けるよう工夫してください。
