...
## 3.6. GPUのCOMPUTE MODE変更 {#GPU-COMPUTE-MODE-change}

資源タイプFullのrt_Fを利用した場合、GPUのCOMPUTE MODEを変更することができます。利用可能なモードは以下の3つです。

| モード | 説明 |
|:--|:--|
| 0 | DEFAULTモード<BR>1つのGPUを複数のプロセスから同時に利用できる。 |
| 1 | EXCLUSIVE_PROCESSモード<BR>1つのGPUを1プロセスのみが利用できる。1プロセスから複数スレッドの利用は可能。 |
| 2 | PROHIBITEDモード<BR>GPUへのプロセス割り当てを禁止する。 |

インタラクティブジョブで利用する場合は、ジョブを投入する際に `-v GPU_COMPUTE_MODE=[利用するモード] /bin/bash` を指定します。
```
[username@es1 ~]$ qrsh -g grpname -l rt_F=1 -l h_rt=1:00:00 -v GPU_COMPUTE_MODE=[利用するモード] /bin/bash
```
バッチジョブで利用する場合は、rt_Fを指定した上で、ジョブスクリプトの中で `#$ -v GPU_COMPUTE_MODE=[利用するモード]` を指定してください。
```
#!/bin/bash
...
### 3.7.1. 予約の実行 {#make-a-reservation}
...
### 3.7.2. 予約状態の確認 {#show-the-status-of-reservations}
...
### 3.7.3. 予約の取り消し {#cancel-a-reservation}
