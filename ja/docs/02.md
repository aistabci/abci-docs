...
例1) オプション指定なしで、所属するABCIグループの直近の利用状態を確認できます。
```
[username@es1 ~]$ show_cs_usage
Cloud Storage Usage for ABCI group GROUP1
  Group                 used(GiB)
  GROUP1                      162
```

例2) オプション -d で日付を指定できます。日付の書式は、 yyyymmdd で指定してください。
```
[username@es1 ~]$ show_cs_usage -d 20191217
Cloud Storage Usage for ABCI group GROUP1 at 2019/12/17
  Group                 used(GiB)
  GROUP1                      124
```
