...
!!! note
    In the case of ABCI cloud Storage, ABCI points of each group are calculated based on the amount of ABCI Cloud Storage usage of the previous day. 
...
## 2.9. Check ABCI Cloud Storage Usage  {#check-cloud-storege-usage}

To display your ABCI Cloud Storage usage, use the `show_cs_usage` command

Example) Display ABCI Cloud Storage information.
```
[username@es1 ~]$ show_cs_usage
Cloud Storage Usage for ABCI group GROUP
  Group                 used(GiB)
  GROUP                       124
```
