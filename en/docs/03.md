...
To submit or execute a job, specify the following resource type name. The ABCI system has two types of computational resources, [compute node](01.md#compute-node) and [memory-intensive node](01.md#memory-intensive-node), multiple resource types corresponding to each mode are prepated as shown in the table below.
