...
## 3.2. Job Execution Resource {#job-execution-resource}
...
The ABCI system has two types of computational resources, [compute node](01.md#compute-node) and [memory-intensive node](01.md#memory-intensive-node), multiple resource types corresponding to each mode are prepated as shown in the table below.
...
With Spot service, you can give your job a priority over other jobs by specifying a POSIX priority.

| Service | Description | POSIX priority | POSIX priority coefficient |
|:--|:--|:--|:--|
| Spot | default  | -500 | 1.0 |
|      | high priority | -400 | 1.5 |

## 3.3. Job Execution Options {#job-execution-options}
...
| -l docker<br>-l docker_images | Submit a job with a Docker container. See [Docker](09.md#docker) for details. |
...
To run an interactive job using the compute resources reserved by Reserved service, specify the reservation ID with the `-ar` option.

Example) Executing an interactive job (Reserved service)
...
To run a batch job using the compute resources reserved by Reserved service, specify the reservation ID with the `-ar` option.

Example) Submission job script run.sh as a batch job (Reserve service)
...
## 3.6. Advance Reservation {#advance-reservation}
