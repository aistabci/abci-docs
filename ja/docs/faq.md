...
## Q. クラウドストレージアカウントを削除したい

クラウドストレージアカウントの削除を希望される方は、[お問い合わせ](./contact.md)のページをご覧の上、<qa@abci.ai> まで削除希望のクラウドストレージアカウント名をご連絡ください。ABCIサポートにて削除処理を行います。削除の動作としては以下のようになりますため、事前に確認ください。

!!! note
    - 削除したクラウドストレージアカウントを戻すことはできません。クラウドストレージアカウント名に間違いがないことを確認してください。
    - クラウドストレージアカウントを削除すると、該当のクラウドストレージアカウントに設定されていたアクセスキーは無効になります。
    - 削除したクラウドストレージアカウントの suffix は欠番となります。例えば、1つのグループに所属している利用者が aaa00000aa.1、aaa00000aa.2 の2つのクラウドストレージアカウントを持ち、aaa00000aa.2 を削除した場合、次に作成されるクラウドストレージアカウントは aaa00000aa.3 になります。「各利用者が持つことのできるアカウントは、所属する1グループあたり最大10個まで」であるので、削除後は aaa00000aa.2 を欠番として aaa00000aa.11 まで作成可能となります。
