...
### ABCI外で作成したDockerの利用方法 {#use-another-docker-image}

ABCIシステム上でABCIシステム外で作成したDockerイメージを利用する方法は以下の2通りがあります。

**パブリックなDockerイメージを作成**

ABCIシステム外でDockerコンテナイメージを作成し、Docker Hubにアップロード
...
ジョブ中に`singularity run`コマンドを実行しSingularityコンテナを起動
...
**DockerfileをSingularityレシピに変換し、Singularityイメージファイルを作成**
