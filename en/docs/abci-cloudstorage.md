...
The fee of the storage is charged to ABCI group that owns the object by each day, based on the size of data stored in the storage of the previous day.  The fee is not based on the data transfer size nor the number of times of calling APIs. The calculation formula of ABCI point for using ABCI Cloud Storage is as follows.
