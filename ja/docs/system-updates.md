...
* ABCI 2.0 インタラクティブノードは今までどおりログイン可能です。
* ABCI 2.0 計算ノード、ローカルストレージ、グローバルスクラッチ領域(/scratch) は利用不可となります。
* ABCI 2.0 インタラクティブノードから、グループ領域(/groups), 高速データ領域(/bb), ABCIクラウドストレージ等のABCI 2.0 ストレージは、今まで同様にアクセス可能です。
    * ただしストレージは段階的に引退します。引退時期は、「[今後のストレージサービスのお知らせ](https://abci.ai/news/2024/09/04/ja_storage_after_nov2024.html)」を参照ください。
* ABCI 2.0 インタラクティブノードにABCI 3.0の /groups 領域が、/groups-new のマウントポイントでマウントされます。
    * /groups, /bb 等のABCI 2.0 ストレージ上の必要なデータは、インタラクティブノードを用いてご自身で /groups-new へデータ移行してください。
* ABCI 2.0 /homeから ABCI 3.0 /homeへのデータ移行はシステム運用者側が実施します。データ移行中はABCI 2.0 インタラクティブノードへのログイン不可となります。その日程は後日お知らせします。

### 12月の更新予定（日程調整中）

* ある時点から、ABCI 2.0 インタラクティブノードには、ABCI 3.0 /home がマウントされます。その日付は後日お知らせします。

### 11月以降のABCI利用者ポータルの制限

* 以下の申請は受付し、ABCI 2.0システムには反映しますが、ABCI3.0へ反映しません。
    * クラウドストレージ管理
* 以下の申請は受付し、ABCI 2.0システムには反映しますが、ABCI3.0への反映は、即時反映しません。ABCI3.0へ反映されるためには別途申請が必要です。その手順は、別途お知らせします。
    * 利用者の編集（メールアドレスの変更のみ）
    * 利用者の削除
    * 公開鍵登録、公開鍵削除
* 以下は申請を受け付けません。利用者ポータル（利用グループ管理）では、下記のボタンは非表示となります。または申請しても承認されません。
    * ABCIポイント追加申請
    * 利用テーマ・利用概要変更申請
    * ノード予約、キャンセル
    * ABCIポイント追加履歴
