...
なお、2021年8月11日以降、旧領域は読み取り専用となっております。

また、2021年度に新規に作成されたABCIグループについては、**新領域**のみが割り当てられるためデータ移行の対象ではなく、データ移行の影響を受けません。


### 基本的な方針 {#basic-strategy}

* **旧領域**のデータを**新領域**に運用側がバックグラウンドでコピーします。全利用者データのコピー完了は2021年度末の予定です。
* 2021年8月11日以降**旧領域**は読み取り専用となっているので、今後は**新領域**を利用してください。
* 移行作業が全て完了した領域は、**新領域**内の移行先へのシンボリックリンクが作成され**旧領域**と同じパスによる参照が可能になります。
* データの移行先は以下の通りです。
...
* データコピーにともないディスク使用量が増加します。このため、移行期間中は、利用者ポータルで申請したグループディスク量(以降、クォータ値)の2倍の値を、**新領域**のディスク使用量上限値に設定します。移行完了後、一定の**猶予期間**を設けて、**新領域**のディスク使用量上限値を、クォータ値と同じ値に設定します。詳細は[グループディスクのクォータ値の変更申請について](faq.md#changing-the-group-disk-quota-value)をご覧ください。


### 旧領域`/groups1/gAA50NNN` および `/fs3/d00[1-2]/gAA50NNN` について

* **旧領域**`/groups1/gAA50NNN` および `/fs3/d00[1-2]/gAA50NNN`は、2021年8月11日以降は読み取り専用となりました。今後は**新領域**に書き込みお願いします。
* 2021年6月28日までに利用者ポータルで申請したグループディスク量(以後、クォータ値と呼ぶ)が、**旧領域**のディスク使用量上限値として設定されています。
	* 2021年6月28日以降、[グループディスクのクォータ値の変更申請について](faq.md#changing-the-group-disk-quota-value)の変更申請の振る舞いが後述の通り変更されました。
...
### グループディスクのクォータ値の変更申請について {#changing-the-group-disk-quota-value}

* 2021年6月27日までは、利用者ポータルで申請したグループディスク量 (以降、クォータ値) を**旧領域**のディスク使用量上限値に、その2倍の値を**新領域**のディスク使用量上限値に設定していましたが、6月28日以降は**移行期間**中のグループディスクのクォータ値の変更申請の振る舞いが以下のように変更されました。
...
* 移行作業終了後は**旧領域**が利用できなくなり、**猶予期間**終了後は**新領域**の利用上限値がクォータ値と同じ値に戻されます。
* グループディスクの利用で消費する ABCI ポイントは従来同様、クォータ値を基に計算されます。


### 移行作業完了後 {#reorganization-period}

* データ移行完了後は、移行期間中に必要なファイルを利用者自身で事前にコピーしたなどで重複したデータを削除/統合するための**猶予期間**となります。
利用者は、この**猶予期間**中にデータを整理してください。
**猶予期間**終了後は、**新領域**の利用上限値をクォータ値と同一に設定します。<br/>
* `/groups2/` 利用者の**猶予期間**は2021年9月末日までです。10月以降にクォータ値を超えるデータが新領域に保存されている場合、書き込みができなくなります。不要なファイル(重複ファイルなど)を削除するか、[ABCI利用者ポータル](https://portal.abci.ai/user/)から利用グループ管理の一覧画面を開き、グループディスクの追加申請を行ってください。</br>
* その他の旧領域の利用者の**猶予期間**は別途ご連絡いたします。
...
| グループ領域            | 状況               | ディスク使用量上限値の猶予期間 |
|:--                      |:--                 |:--                             |
| `/groups1/gAA50NNN/`    | 移行中             | 移行完了後に設定               |
| `/groups2/gAA50NNN/`    | 2021年7月1日に完了 | 2021年9月30日まで              |
| `/fs3`                  | 2021年10月開始予定 | 移行完了後に設定               |


## Q. 旧グループ領域にアクセスできません {#q-why-unable-to-access-the-files-in-the-old-group-area}
...
* 旧領域は読み取り専用です
	...
	**旧領域**(`/groups1/`および`/fs3/`)は2021年8月11日以降読み取り専用に設定しました。
	今後は**新領域**(`/groups/`または`/projects/`)をご利用ください。<br/>
	なお、移行作業が完了しますと、**旧領域**と同じパスで移行先ディレクトリを参照するシンボリックリンクが作成されるので書き込みも可能になります。
