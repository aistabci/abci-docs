...
* HPC-Xパラメータの修正
    * hpcx*/2.12 モジュールの"hwloc_base_binding_policy"パラメータのデフォルト値が下記のとおり変更されます。<br>
        * 変更前のデフォルト値："core"(CPUコアにプロセスをバインドする)<br>
        * 変更後のデフォルト値："none"(バインドしない)

## 2023-06-02

* 計算ノード(V)および計算ノード(A)のSpotサービスおよびReservedサービスの以下の上限値を変更しました。

| サービス | 資源タイプ | 項目 | 変更前の上限値 | 変更後の上限値 |
|:--|:--|:--|:--|:--|
| Spot | rt_F, rt_G.large | 経過時間上限 | 72時間 | 168時間 |
| Spot | rt_G.small, rt_AG.small, rt_C.small | 経過時間上限 | 168時間 | 72時間 |
| Spot | rt_F | ノード時間積上限 | 2304ノード時間 | 43008ノード時間 |
| Reserved | rt_F | 1予約あたりの最大予約ノード数 | 34ノード | 272ノード |
| Reserved | rt_F | 1予約あたりの最大ノード時間積 | 13056ノード時間 | 45696ノード時間 |
| Reserved | rt_F | システムあたりの最大同時予約可能ノード数 | 442ノード | 476ノード |
