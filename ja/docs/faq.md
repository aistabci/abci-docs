...
* 利用者はなるべく**新領域**を利用してください。**旧領域**`/groups2/gAA50NNN/` のパスもそのままご利用いただけますが、これは**新領域**内の移行先`/groups/gAA50NNN/migrated_from_SFA_GPFS/` へのシンボリックリンクであり、**旧領域**内にデータが残っている訳ではないことに注意してください。
...
* 移行作業中は、**旧領域**からコピーされてくるデータが急増するため、一時的に申請されたクォータ値をある程度超えてもご利用いただけるように設定されておりました。
...
| 読み取り専用対応のための「事前準備」| 不要 | 8月12日までに書き込み先の変更が必要 |
