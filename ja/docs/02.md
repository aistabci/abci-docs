...
### テーマとグループ {##themes-and-groups}
ABCIを利用するためには、明確な利用目的をもったテーマを決定して利用申請する必要があります。まず[「ご利用の流れ」](https://abci.ai/ja/how_to_use/)に掲載された手順に従って産総研に「ABCI利用申請」を提出していただきます。産総研では提出されたテーマの内容を審査して採択された後、そのテーマに取り組むメンバーからなるグループを作成し、申請者に採択された旨をお知らせいたします。

テーマが採択された通知を受け取ったら、実施に必要なポイントを購入していただくことで運用が開始されます。
ポイントはグループごとに購入していただくことになります。

### メンバーの種類 {##member-type}
グループに所属するメンバーには、「利用責任者」「利用管理者」「利用者」の3種類があります。ABCIシステムを利用するには、「利用責任者」が[ ABCI利用ポータル ](https://portal.abci.ai/user/project_register_app.php)からABCIグループ申請・利用者(「利用管理者」もしくは「利用者」)を行い、ABCIアカウントを取得する必要があります。
...
### ABCIアカウントの一意性
1人の方は原則として1つのABCIアカウントを取得していただくことになります。1つのアカウントを複数の方で共有して使い回すことは認められません。
一方で1人の方が複数の団体に所属しており、それぞれ異なった肩書をお持ちの場合は複数のABCIアカウントを取得することができます。この場合、それぞれの肩書に対応するアカウントを使い分けていただく必要があります。

また、1人の方が1つの団体の中で複数のテーマに同時に取り組んでいる場合、1人で複数のアカウントをそれぞれ取得するのではなく、1つのアカウントで複数のグループに所属するメンバーとなります。この場合、その利用目的に応じてグループを使い分けていただく必要があります。

このルールは、ご利用料金をどのグループが負担するかに関わってくるので、厳格に守ってください。
