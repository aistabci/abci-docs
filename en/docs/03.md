...
## 3.6. GPU COMPUTE MODE {#GPU-COMPUTE-MODE-change}

Only when using resource type rt_F, you can change GPU compute mode. The following three modes are available.

| MODE | Description |
|:--|:--|
| 0 | DEFAULT mode<BR>Multiple contexts are allowed per device. |
| 1 | EXCLUSIVE_PROCESS mode<BR>Only one context is allowed per device, usable from multiple threads at a time. |
| 2 | PROHIBITED mode<BR>No contexts are allowed per device (no compute apps). |

To execute an interactive job, specify the `-v GPU_COMPUTE_MODE=[MODE] /bin/bash`.
```
[username@es1 ~]$ qrsh -g grpname -l rt_F=1 -l h_rt=1:00:00 -v GPU_COMPUTE_MODE=[MODE] /bin/bash
```
...
### 3.7.1. Make a reservation {#make-a-reservation}
...
### 3.7.2. Show the status of reservations {#show-the-status-of-reservations}
...
### 3.7.3. Cancel a reservation {#cancel-a-reservation}
