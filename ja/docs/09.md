...
オリジナルのDockerfileをそのまま変換した場合には、ビルド時に次の2点の問題が発生するため、Dockerfileを修正してから変換します。
...
FROM ${FROM_IMAGE_NAME}
# Add pip Path                           <- 追加
ENV PATH "/opt/conda/bin:${PATH}"        <- 追加
# Set working directory
WORKDIR /workspace
ENV PYTHONPATH "${PYTHONPATH}:/workspace"
COPY requirements.txt /workspace         <- コピー先を相対パス（.）から絶対パスに変更
RUN pip install --no-cache-dir git+https://github.com/NVIDIA/dllogger.git#egg=dllogger
RUN pip install -r requirements.txt
RUN python3 -m pip install pycocotools==2.0.0
# Copy SSD code
COPY ./setup.py /workspace               <- コピー先を相対パス（.）から絶対パスに変更
COPY ./csrc /workspace/csrc              <- コピー先を相対パス（./csrc）から絶対パスに変更
RUN pip install .
COPY . /workspace                        <- コピー先を相対パス（.）から絶対パスに変更
