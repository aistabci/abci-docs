...
|:--|:--|:--|:--|
| On-demand | インタラクティブジョブの実行サービス | 1.0 | インタラクティブ利用 |
| Spot | バッチジョブの実行サービス | 1.0 | バッチ利用 |
| Reserved | 事前予約型ジョブサービス | 1.5 | 事前予約 |

各ジョブサービスで利用可能なジョブ実行リソース、制限事項等については、[ジョブ実行リソース](#job-executing-resource)を参照してください。また、課金については、[課金](#accounting)を参照してください。
...
利用方法は[バッチジョブ](#interactive-jobs)、バッチジョブの実行オプションの詳細は[ジョブ実行オプション](#job-execution-options)をそれぞれ参照してください。
...
予約方法は[事前予約](#reservation)、インタラクティブジョブやバッチジョブの利用方法、実行オプションはOn-demandおよびSpotサービスと共通です。
...
また、SpotおよびOn-demandサービスで、複数ノードを使用するジョブを実行する場合には、ノード時間積（使用ノード数 &times; 実行時間）に以下の制限があります。
...
> [サービス課金係数](#job-services) &times; [資源タイプ課金係数](#available-resource-types) &times; [POSIX 優先度課金係数](#execution-priority)  
> &times; 資源タイプ個数 &times; max(経過時間[秒], 最低経過時間[秒]) / 3600
...
> [サービス課金係数](#job-services) &times; 予約ノード数 &times; 予約日数 &times; 24
