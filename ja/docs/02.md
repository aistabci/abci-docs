...
!!! note
    ABCIクラウドストレージのABCIポイントは、ABCIグループ毎に前日のABCIクラウドストレージの使用量から算出されたポイントが表示されます。
...
## 2.9. ABCI クラウドストレージ利用状況の確認 {#check-cloud-storege-usage}

ABCI クラウドストレージの使用状況表示するには、`show_cs_usage` コマンドを利用します。

例) ABCIグループ GROUP のクラウドストレージの利用状態を確認する。
```
