...
!!! note
    実行中のジョブにより指定された期間で予約可能なノード数が`qrsub`コマンドで指定したノード数より少ない場合、以下のエラーメッセージを出力して予約取得に失敗します。  
    ```
    advance_reservation: no suitable queues
    ```  
    なお、`qrstat --available` コマンドで表示されるノード数には、現在実行中のSPOTジョブおよび、その時間帯に実行する予定のジョブは加味されておりません。そのため、qrstat コマンドで「予約されていないノード」が表示されていても、予約に失敗することがあります。  
