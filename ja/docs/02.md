...
* 安全なSSH公開鍵・秘密鍵ペアの生成。ABCIで利用可能な鍵ペアは以下のとおりです。
	...
	* Ed25519鍵
* SSH公開鍵の登録。[ABCI利用ポータル](https://portal.abci.ai/user/)にてSSH公開鍵の登録を事前に行ってください。登録方法は、[SSH公開鍵の登録](https://portal.abci.ai/docs/portal/ja/02/#23)を参照してください。
...
さらにローカルPCの``$HOME/.ssh/config``に以下の記述を行うと、``ssh abci``とタイプするだけで実行できます。
