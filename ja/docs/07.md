...
資源タイプFullを利用した場合、GPUのCOMPUTE MODEを変更することができます。
選択できるMODEの詳細は[GPUのCOMPUTE MODE変更](https://docs.abci.ai/ja/03/#gpucompute-mode) を参照ください。

インタラクティブ利用の場合は、ジョブを投入する際に `-v GPU_COMPUTE_MODE=[利用するモード]` を指定します。また、ノードを占有する必要があるため、`-l rt_F`オプションを指定する必要もあります。

```
[username@es1 ~]$ qrsh -g grpname -l rt_F=1 -l h_rt=1:00:00 -v GPU_COMPUTE_MODE=1 /bin/bash
```

バッチ利用の場合は、rt_Fを指定した上で、ジョブスクリプトの中で `#$ -v GPU_COMPUTE_MODE=[利用するモード]` を指定してください。
