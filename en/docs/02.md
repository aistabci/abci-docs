...
To connect to the interactive node (*es*), the ABCI frontend, you first need to register your SSH public key on [ABCI User Portal](https://portal.abci.ai/user/).  After that, you need to login to the access server (*as.abci.ai*) with SSH publickey authentication, so as to create an *SSH tunnel* between your local computer and *es*.
...
This section describes two methods to login to the interactive node. The first one is first creating an SSH tunnel on the access server, and connecting the interative node via this tunnel next. The second one, much easier, is connecting directly to the interactive node using ProxyJump implemented in OpenSSH 7.3 or later.
...
If you add the following configuration to your ``$HOME/.ssh/config``, you just need to type ``ssh abci``:
...
ProxyJump is available on your computer, you can directly run the `scp` (`sftp`) command.
